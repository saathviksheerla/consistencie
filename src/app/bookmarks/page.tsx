            ...
            {bookmarks.length === 0 ? (
                <Card className="border-dashed">
                    <CardContent className="flex flex-col items-center justify-center p-12 text-center text-muted-foreground">
                        ...
                        <Button variant="outline" onClick={() => setIsAddModalOpen(true)}>Save your first link</Button>
                    </CardContent>
                </Card>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {/* Pinned Bookmarks first */}
                    {[...bookmarks].sort((a, b) => Number(b.isPinned) - Number(a.isPinned)).map((bookmark) => (
                        <Card key={bookmark.id} className={`flex flex-col group relative ${bookmark.isPinned ? 'border-accent/50 bg-accent/5' : ''}`}>
                            <CardHeader className="pb-3 border-b border-border/50">
                                <div className="flex justify-between items-start gap-3">
                                    <div className="flex-1 min-w-0">
                                        <CardTitle className="truncate font-medium text-base mb-1.5" title={bookmark.title}>
                                            <a href={bookmark.url} target="_blank" rel="noopener noreferrer" className="hover:text-accent transition-colors">
                                                {bookmark.title}
                                            </a>
                                        </CardTitle>
                                        <Badge variant={getCategoryColor(bookmark.category) as any}>{bookmark.category}</Badge>
                                    </div>
                                    <button
                            ...
                            <CardContent className="pt-3 flex flex-col justify-end mt-auto">
                                <div className="flex justify-between items-center">
                                    <a href={bookmark.url} target="_blank" rel="noopener noreferrer" className="text-xs text-muted-foreground truncate hover:text-foreground hover:underline max-w-[70%]">
